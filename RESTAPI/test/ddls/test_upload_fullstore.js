...
function deleteStore(url, name)
{
    ...
    try {
        request.del(url+name).proxy(prox).end(function(e, r) {
            if (e) {
                defer.resolve();
            }
            else
...
function reCreateStore(url, name)
{
    ...
    deleteStore(url, name).then(function() {
        request.put(url+name).proxy(prox).end(function(e, r) {
            if (e) {
                console.log(e);
                defer.resolve();
            }
...
function addGlobalDDLObject(url, store, name, content)
{
    ...
    request.put(url+store+"/"+name).proxy(prox).send( {type:'ddl',
                                                       content: content,
...
function addDDLObject(url, store, name, content)
{
    ...
    request.put(url+store+"/"+name).proxy(prox).send( {type:'ddl',
                                                content: content,
                                                global:false} ).proxy(prox).end(function(e, r) {
                                                    if (e) {
                                                        console.dir(r.error);
                                                        throw e;
                                                    }
...
function addGlobalFiles(arr) {
    try {
        function addGlobalFile( idx ) 
        {
            ...
            var defer = q.defer();
            var url = 'inductum.com:8082/api/metastore/'; 
...
function deleteFile(name) 
{
    ...
    try {
        request.del(url+'mystore'+"/"+name).proxy(prox).end(function(e, r) {
            if (e) {
                //throw e;
            }
...
//reCreateStore('inductum.com:8082/api/metastore/', 'mystore').then( function() {
